...
- Safety warnings are generated by deterministic modifier and retrieval
  safeguards. Address them explicitly in the classification reasoning and do
  not treat a suggested base alias as approval of the full uploaded phrase.
...
EvidenceNotes = Sequence[Sequence[str]]
...
class ClassificationRequest:
    ...
    evidence_notes: EvidenceNotes | None = None
...
def build_user_message(
    ...
    for i in range(1, len(raw_values) + 1):
        ...
        lines.append(json.dumps({
            "input_index": i,
            "evidence_status": (
                "CONFLICTING" if len(labels) > 1
                else "CAUTION" if notes
                else "CONSISTENT"
            ),
            "observed_canonical_labels": labels,
            "safety_warnings": [str(note)[:500] for note in notes],
            "approved_mappings": [
                {
...
def build_verification_message(
    ...
    for local_index, original_index in enumerate(original_indexes, start=1):
        ...
        notes = (
            normalized_notes[original_index - 1]
            if original_index <= len(normalized_notes)
            else ()
        )
        if not examples and not notes:
            continue
...
def classify_values(
    ...
    user_message = build_user_message(raw_values, approved_examples, evidence_notes)
...
def classify_request(
    ...
    notes: EvidenceNotes = request.evidence_notes or [
        [] for _ in raw_values
    ]
